...
class mercedesSpider(scrapy.Spider):
    name =  "mercedes_italy15_02_new" #"mercedes_italy_deatil_page26-01"#mercedes_italy25-01-firstcache(old)
      ...
      #  for i in range(4,6):#try with splashRequest and autoload jquery
        ...
        for i in range(1,2):   #230
            ...
            formdata = {'action':'getPage','brand':'MERCEDES','dealer':'all','carline':'all',
                    ...
                    'sortBy':'price','carsPerPage':'2000','special_query':'all'
                }
   ...
   #         yield scrapy.Request(links, callback=self.parse)
    ...
    def parse(self, response):
           ...
           # for info in data.get('results',[]).get('list',[]).get('paged',[]).get('cars',[]):#get url from json first
            ...
            bb = data.get('results').get('list').get('paged').get('cars')
            #for info in bb:#.iterkeys():

                #myItem = MercedesItem()
                #myItem['ANNONCE_LINK'] = info#data
                #myItem['TELEPHONE'] = type(info)
                #myItem['TELEPHONE'] = type(info)
                #myItem["NOM"] = info.get('title')
            details = data.get('results',[]).get('list',[]).get('paged',[]).get('cars',[])
            #myItem = MercedesItem()
            #for info in bb:
             #   myItem = MercedesItem()
            #    all_cars_details = info
               # myItem['ANNONCE_LINK'] = bb#all_cars_details#re.search(r':(.*?)', all_cars_details).group(1)
            with open("2.csv", "w") as text_file: 
                    myItem = MercedesItem()
                    text_file.writelines(str(bb))
                    print bb
                    #short_title
                    os.system("egrep  -o 'http?://usato.firsthand.it/a[^ ]+' 2.csv > 2_links.csv") #True
                    #detail':
#                    os.system("egrep  -o 'detail:[^ ]+' 2.csv > 2_links.csv")
                    #cc = re.search(r'short_title(.*?),', bb).group(1)
                    #print cc >> 'links17_uniq.csv'
                    #os.system("egrep  -o 'http?://usato.firsthand.it/a[^ ]+' Output.csv > links17.csv")
                    #os.system("sort links17.csv | uniq -c > links17_uniq.csv")
                #os.system("egrep  -o 'http?://usato.firsthand.it/a[^ ]+' info16.csv > links16.csv")
                #os.system("sort links16.csv | uniq -c > links16_uniq.csv")
                    urls_file = [l.strip() for l in open('/home/databiz41/scrapy_projects/mercedes/mercedes/2_links.csv').readlines()]
                    for url in urls_file:#self.start_urls:
                        urls = url.split("'")
                        link = urls[0]
                        link2 = link.split(' ')
                        cc = link2[-1]
                 ...
                 #   link = links[-1].split(' ')
                 #   cc = link[-1]
                #   # yield scrapy.Request(links, callback=self.parse_phone)
                    #myItem['TELEPHONE'] = hh

                        request = scrapy.Request(cc, callback=self.parse_phone)
                        request.meta["myItem"] = myItem
                        yield request
               # yield scrapy.Request(cc, callback=self.parse_phone) 
               # yield myItem


            #    myItem['TELEPHONE'] = re.search(r'title(.*?),', info).group(1)
                #myItem['ANNONCE_LINK'] = info#data.get('results').get('list').get('paged').get('cars')
                #with open('out.csv', 'w') as f:
                    #print >> f
                #myItem['TELEPHONE'] = type(myItem['ANNONCE_LINK'])#re.search(r"detail myItem['ANNONCE_LINK']).group(1)
#                os.system("egrep  -o 'http?://usat[^ ]' info2.csv > links20_0.csv")
                    
           # for info in  data.get('results').get('list').get('paged').get('cars'):
                #y = json.dumps(info)
                #b = ast.literal_eval(info)
                #c = json.loads(y)
            #    myItem = MercedesItem()
                #myItem['TELEPHONE'] = re.search(r"title(.*?)'", info).group(1)
                #re.search(r"detail(.*?)'", l).group(1)
             #   myItem['ANNONCE_LINK'] = info    #all_cars_details#re.search(r'title: (.*?),', info)#.group(1) #c.get('results',[])#.get('list',[]).get('paged',[]).get('cars')#['title']#.title#('title')
                
            #for cc in data:
            #    myItem = MercedesItem() 
             #   myItem['TELEPHONE'] = cc.get('results',[]).get('list',[]).get('paged',[]).get('cars',[])
                #yield myItem
                #time.sleep(1)
                #os.system("egrep  -o 'http?://usato.firsthand.it/a[^ ]+' info7.csv > links7.csv")
#                detail': u'http://usato.firsthand.it/auto/
                #http://usato.firsthand.it/auto
                #egrep  -o 'http?://usat[^ ]+' links.csv > links2.csv
           # json_response = data.get('results',[])
           # for info_all in json_response:  
           #     myItem = MercedesItem()
           #     myItem['TELEPHONE'] = info_all.get('list').get('paged').get('cars')
#            for info in data.get('results').get('list').get('paged').get('cars',[]):
...
#                myItem = MercedesItem()
 #               myItem['ANNONCE_LINK'] = info
  ...
  #              y = json.dumps(info)
                #for all in info:
   #             myItem['TELEPHONE'] = info.get('title')
                 #    myItem['NOM'] = re.search(r'title": u"(.*?)', all)#.group(1)
#
 #               myItem['NOM'] = re.search(r'u"color_name": u"(.*?)"', info)#.group(1) 
               ...
               # myItem['PROVINCE'] =  re.search(r'addressRegion": "(.*?)"', response.body ).group(1)
                #locality =  re.search(r'"addressLocality": "(.*?)"', response.body ).group(1)#was VILLE changed09-20 
                #myItem['DEPARTEMENT'] = locality +' '+ '('+myItem['PROVINCE']+')'
                #myItem['DEPARTEMENT'] = " "#deleted byCyrielle
...
#                myItem['OPTIONS'] = response.xpath('//meta[@property="og:description"]/@content').extract()
 ...
 #               myItem['BOITE'] = response.xpath('//ul[@class="last"]/li[4]/span/text()').extract_first()
    ...
    def parse_phone(self, response):
        ...
        cc = response.xpath('//div[@class="col-content"]/p[2]/text()').extract_first()
        bb = cc.split(':')
        myItem['TELEPHONE'] = bb[-1].strip()  # dealer_phone to TELEPHONE
        myItem['ADRESSE'] =  re.search(r'streetAddress": "(.*?)"', response.body ).group(1)
        myItem['PROVINCE'] = " "#  re.search(r'addressRegion": "(.*?)"', response.body ).group(1)
        ...
        ville = myItem['ADRESSE'].split(',')
        myItem['PUISSANCE'] = response.xpath('//ul[@class="last"]/li[1]/span/text()').extract_first() 
        myItem['GARAGE_NAME'] = response.xpath('//div[@class="col-full feedback map"]/div/h1/text()').extract_first() #change dealer_name to garage_name
        myItem['GARAGE_NAME'] = myItem['GARAGE_NAME'].replace('Concessionario: ','')
        myItem['FROM_SITE'] = 'http://usato.firsthand.it'
        myItem['LONGITUDE'] = re.search(r'var lon = (.*?);', response.body).group(1)
        myItem['LATITUDE'] = re.search(r'var lat = (.*?);', response.body).group(1)
        myItem['EMAIL'] = response.xpath('//input[@id="dealer_email"]/@value').extract() #from/auto change DEALER_EMAIL to email
        ...
        myItem['CARBURANT'] = response.xpath('//li[@class="last"]/span/text()').extract_first()
        myItem['CYLINDRE'] = response.xpath('//ul[@class="first"]/li[3]/span/text()').extract_first()
        myItem['CYLINDRE'] = myItem['CYLINDRE'].replace('cc','')
        myItem['KM'] = response.xpath('//div[@class="col-lg-12 col-md-12 col-sm-6 col-xs-6"]/li[2]/p/span/text()').extract_first()
        myItem['KM'] = myItem['KM'].replace('km','').strip()
        myItem['PRIX'] = response.xpath('//li[@id="bloccoPrezzo"]/p/span/text()').extract_first()
        myItem['PRIX'] = myItem['PRIX'].split(' ')
        myItem['PRIX'] = myItem['PRIX'][0]
        myItem['COULEUR'] = response.xpath('//ul[@class="last"]/li[2]/span/text()').extract_first()
        myItem['BOITE'] = response.xpath('//ul[@class="last"]/li[4]/span/text()').extract_first()
